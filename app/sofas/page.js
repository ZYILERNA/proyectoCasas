...
const ProductCard = memo(({ item, index, onSelect }) => {
    ...
    const cardImage = useMemo(() => {
        ...
        return item.image || "/images/sofa-header.webp";
    }, [item, hoverColorIdx]);
        ...
        >
            ...
            <div className="flex flex-col gap-1">
                <div className="flex justify-between items-baseline">
                    <button
                        type="button"
                        className="text-left text-lg font-medium text-gray-900 leading-tight group-hover:text-gray-600 transition-colors cursor-pointer focus-visible:outline focus-visible:outline-2 focus-visible:outline-black"
                        onClick={() => onSelect(item)}
                    >
                        {item.name}
                    </button>
...
function SofasContent() {
    ...
    useEffect(() => {
        ...
        let active = true;
        (async () => {
            const { data } = await supabase.from('sofas').select('*').eq('name', producto).limit(1);
            ...
            <div className="relative h-[55vh] md:h-[65vh] bg-[#0a0a0a] overflow-hidden flex items-end pb-12">
                ...
                >
                    <OptimizedVideo
                        ...
                        sources={[
                            { src: "/videos/sofa-hero.mp4", type: "video/mp4" },
                        ]}
                    />
                </motion.div>
