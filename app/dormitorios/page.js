...
import { X, ChevronLeft, ChevronRight, ScanLine, Ruler, Box, Palette, RefreshCw, Lock, Layers, BedDouble, Maximize2, Barcode, Search } from 'lucide-react';
...
const ProductCard = memo(({ item, index, onSelect }) => {
    ...
    const cardImage = useMemo(() => {
        ...
        return item.image || "/images/dormitorios-header.webp";
    }, [item, hoverColorIdx]);
...
function DormitoriosContent() {
  ...
  useEffect(() => {
    ...
    let active = true;
    (async () => {
      const { data } = await supabase.from('dormitorios').select('*').eq('name', producto).limit(1);
  ...
  useEffect(() => {
    ...
    async function fetchDormitorios() {
      setIsLoading(true);
      try {
        ...
        if (error.name !== 'AbortError') console.error("Error cargando dormitorios:", error.message);
      } finally {
        setIsLoading(false);
      }
    }
      ...
      <div className="relative h-[35vh] md:h-[65vh] bg-[#0a0a0a] overflow-hidden flex items-end pb-8 md:pb-12">
        ...
        >
            ...
            <motion.p variants={fadeInUp} className="text-gray-300 max-w-xl text-sm md:text-base">
              Descanso, confort y un diseño pensado para la desconexión total.
            </motion.p>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 py-6 md:py-12" ref={gridTopRef}>
        {isLoading ? (
            ...
            <div className="text-center py-32 text-gray-400 flex flex-col items-center">
                ...
                <p className="text-lg">No encontramos dormitorios.</p>
