...
import { X, ChevronLeft, ChevronRight, ScanLine, Ruler, Box, Palette, RefreshCw, Lock, Layers, BedDouble, Maximize2, Barcode, Loader2, Search } from 'lucide-react';
...
const isSimpleFormat = data.some(row => row.price !== undefined || row.prices?.unique !== undefined);
                                ...
                                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    {isSimpleFormat ? (
                                        <td className="p-3 text-center font-bold text-emerald-700 bg-emerald-50/30">
                                            {formatPrice(row.price || row.prices?.unique)}
                                        </td>
                                    ) : (
                                        <>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 md:p-8 pt-6 pb-32 md:pb-48 scrollbar-hide">
                            <div className="space-y-8">
                                ...
                                <div>
                                    ...
                                    {Object.values(selectedProduct.colors).flat().map((color, i) => (
...
const { data, error } = await supabase
  ...
  .abortSignal(controller.signal);
        ...
        if (error.name !== 'AbortError') {
             console.error("Error cargando gabinetes:", error.message);
        }
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="relative h-[55vh] md:h-[65vh] bg-[#0a0a0a] overflow-hidden flex items-end pb-12">
        ...
        >
            ...
            <motion.p variants={fadeInUp} className="text-gray-300 max-w-xl text-sm md:text-base">
              Almacenamiento, organización y un diseño pensado para optimizar tus espacios.
            </motion.p>
        </motion.div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="relative group w-full md:w-72 shrink-0">
                    ...
                    <input 
                        ...
                        className="w-full bg-[#F5F5F5] border border-transparent focus:bg-white focus:border-gray-200 rounded-full py-2.5 pl-11 pr-4 text-xs font-bold uppercase tracking-wide focus:ring-0 transition-all outline-none text-black placeholder:text-gray-400"
                    />
                    {searchTerm && (
                        <button 
